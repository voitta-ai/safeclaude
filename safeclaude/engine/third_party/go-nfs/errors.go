...
func basicErrorFormatter(err error) RPCError {
	// safeclaude patch: prefer the innermost NFS status (see
	// errFormatterWithBody).
	var nerr *NFSStatusError
	if errors.As(err, &nerr) {
		var inner *NFSStatusError
		if errors.As(nerr.WrappedErr, &inner) {
			return inner
		}
	}
	var rpcErr RPCError
...
func errFormatterWithBody(body []byte) func(err error) RPCError {
	return func(err error) RPCError {
		if nerr, ok := err.(*NFSStatusError); ok {
			// safeclaude patch: handlers hardcode NFSStatusAccess around fs
			// errors; if the filesystem itself returned an NFS status (e.g.
			// jukebox to signal "approval pending, client please retry"),
			// pass that through instead of the handler's blanket mapping.
			var inner *NFSStatusError
			if errors.As(nerr.WrappedErr, &inner) {
				nerr = inner
			}
